...
def inference(images):
    ...
    with tf.variable_scope('conv1') as scope:
        kernel = _variable_with_weight_decay(
            'weight', shape=[5, 5, 3, 64], stddev=5e-2, wd=None)
        conv = tf.nn.conv2d(images, kernel, strides=[
    ...
    pool1 = tf.nn.max_pool(conv1, ksize=[1, 3, 3, 1], strides=[
                           1, 2, 2, 1], padding='SAME', name='pool_1')
    ...
    with tf.variable_scope('conv2') as scope:
        ...
        conv = tf.nn.conv2d(norm1, kernel, strides=[
                            1, 1, 1, 1], padding='SAME')
    ...
    pool2 = tf.nn.max_pool(norm2, ksize=[1, 3, 3, 1], strides=[
                           1, 2, 2, 1], padding='SAME', name='pool_2')
    ...
    with tf.variable_scope('softmax_linear') as scope:
        weights = _variable_with_weight_decay(
            'weights', [192, NUM_CLASS], stddev=1 / 192.0, wd=None)
        biases = _variable_on_cpu(
            'biases', [NUM_CLASS], tf.constant_initializer(0.0))
