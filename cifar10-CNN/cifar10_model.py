...
BATCH_SIZE = 128
DATA_DIR = '/Users/limingxia/desktop/study/AI/cifar10-CNN/cifar10_data/cifar-10-batches-bin'
...
def inference(images):
    ...
    with tf.name_scope('conv1'):
        conv1_w = tf.Variable(tf.truncated_normal(
            shape=[5, 5, 3, 64], mean=0, stddev=5e-2), name='weights')
        conv1_b = tf.Variable(tf.zeros(64), name='biases')
        conv1 = tf.nn.relu(tf.nn.conv2d(images, conv1_w, strides=[
    ...
    # norm1
    norm1 = tf.nn.lrn(pooling_1, 4, bias=1.0, alpha=0.001 /
                      9.0, beta=0.75, name='norm_1')

    # conv2
    with tf.name_scope('conv2'):
        conv2_w = tf.Variable(tf.truncated_normal(
            shape=[5, 5, 64, 64], mean=0, stddev=5e-2), name='weights')
        conv2_b = tf.Variable(tf.zeros(64), name='biases')
        conv2 = tf.nn.relu(tf.nn.conv2d(norm1, conv2_w, strides=[
    ...
    # norm 2
    norm2 = tf.nn.lrn(pooling_2, 4, bias=1.0, alpha=0.001 /
                      9.0, beta=0.75, name='norm_2')
    ...
    with tf.name_scope('local3'):
        local3_w = tf.Variable(tf.truncated_normal(
            shape=[dim, 384], mean=0, stddev=0.04), name='weights')
    ...
    with tf.name_scope('local4'):
        local4_w = tf.Variable(tf.truncated_normal(
            shape=[384, 192], mean=0, stddev=0.04), name='weights')
    ...
    with tf.name_scope('softmax_linear'):
        output_w = tf.Variable(tf.truncated_normal(
            shape=[192, 10], mean=0, stddev=1 / 192.0))
...
def train_step(loss_value, global_step):
    ...
    optimizer = tf.train.GradientDescentOptimizer(
        learning_rate).minimize(loss_value)
    return optimizer
...
def accuracy(logits, lables):
    ...
    accuracy_value = tf.reduce_mean(
        tf.cast(tf.equal(reshape_logits, lables), dtype=tf.float32))
